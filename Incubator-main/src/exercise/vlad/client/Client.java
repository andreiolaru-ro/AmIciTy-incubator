...
public class Client extends JFrame implements ActionListener
{
	...
	public void init()
	{
		...
		this.setLocation(dim.width / 2 - width / 2, dim.height / 2 - height
				/ 2);
	 ...
	 * 
	 * @param received
	 *             : the String received from the thread
	...
	public synchronized void setText(String received)
	{
		...
		if (status == 0)
		{
			output = Integer.toString((int) (time2 - timeStart) / 100)
					+ ":   " + received;
			GP.repaint();
		}
		else
	...
	public synchronized void actionPerformed(ActionEvent e)
	{
		...
		if (command.equals("Lock Output"))
			// daca a fost blocat:
			// se deblocheaza, se modifica variabila Canvasului pentru a stii
			// ca va primi Lista si nu un string
			if (status == 1)
			{
				status = 0;

				GP.changeAfis();  // GP isi va modifica o variabila pentru a
								// stii ca trebuie a afiseze un
								// ArrayList
				GP.repaint();
			}
			else
